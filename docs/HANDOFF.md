...
| Thing | State |
|---|---|
| **Hosted backend** | **LIVE.** Supabase project `izvcozvfqmggyziaeeoc` (name "Peak Social", `https://izvcozvfqmggyziaeeoc.supabase.co`, us-west-2, PG 17). All 41 migrations applied, `pg_cron` + `pg_net`, 3 retention jobs + `ingest-content` (every 6h), `app-version` / `export` / `publish` / `ingest-content` edge functions deployed, security-hardened. Seeded via `tool/seed-directory.sql`: house `@peak`; mirror/news accounts `@nasa` `@webb` `@hubble` `@roman` `@launches` `@playstation` `@xbox` `@nintendo` `@pcgaming` `@pchardware` `@scinews`; communities `c/space` `c/rockets` `c/gaming` `c/playstation` `c/xbox` `c/nintendo` `c/pc-gaming` `c/pc-hardware` `c/science` `c/science-news` `c/astrophotos`. |
...
| _(this commit)_ | **Gaming/science communities + news feeds; feed video; Discover.** (1) `ingest-content` gains an `rss-news` kind — `@playstation` / `@xbox` / `@nintendo` / `@pcgaming` / `@pchardware` / `@scinews` post a headline + excerpt + link (no image) into `c/playstation` / `c/xbox` / `c/nintendo` / `c/pc-gaming` / `c/pc-hardware` / `c/science-news`. Feeds: PlayStation.Blog, Xbox Wire, Nintendo Life, PC Gamer + RPS, Tom's Hardware + TechPowerUp, Phys.org + ScienceDaily. `seed-directory.sql` extended. (2) **Video**: `video_player` dep; composer "Video" button (1 clip, ≤60 s / ≤50 MB, `kind: 'video'`); `_Video` player in `post_media_view.dart` (tap-to-play, scrub, mute, data-light). Reply/community-post video supported server-side; composer restricts to non-reply. (3) **Discover**: `popularCommunitiesProvider` (via `communities_browse`) + `freshOnPeakProvider` (via `feed_local`) — graph-free sections so a fresh account sees content. Operator interests pre-seeded. (4) **Dialog fix**: `showDialog` action handlers in `compose_screen` / `chat_screen` / `group_settings_screen` were calling `Navigator.pop(context)` with the screen's context (a nested shell navigator) instead of the dialog's — buttons did nothing. Now use the builder's context. |
